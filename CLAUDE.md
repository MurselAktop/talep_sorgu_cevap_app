...
### Faz 3 — Liste kullanılabilirliği
- Talep listelerine arama (başlık/açıklama metni, `ilike`) ve filtreleme (durum, kategori, tarih aralığı).
- **Etkilenen yerler:** `request_list_screen.dart`, `my_requests_screen.dart` (muhtemelen ortak bir filtre widget'ı çıkarılır). Şema değişikliği yok; RLS zaten üst sınırı belirliyor, filtreler istemci tarafı WHERE koşulu.

### Faz 4 — Admin ve müdür yönetim araçları
- Admin birim yönetim ekranı: birim ekleme/düzenleme; silme yerine PASİFLEŞTİRME (`departments.is_active`).
- Talep yönlendirme: müdür, yanlış birime düşen talebi başka birime aktarabilsin (`department_id` güncelleme; müdürün UPDATE RLS'i "kendi birimi" koşullu olduğu için yeni satır değeri politikaya takılır — muhtemelen `security definer` RPC gerekir, faz başında netleştirilecek). Yönlendirme geçmişi tutulup tutulmayacağı KULLANICIYA SORULACAK.
- Hesap pasifleştirme: `users.is_active` (mevcut "Soft Delete Deseni" notuyla birleşir); pasif kullanıcının giriş/işlem yapamamasını en sağlam engelleme yöntemi (Auth `banned_until` vs RLS koşulu vs ikisi birden) araştırılıp önerilecek.
- **Etkilenen yerler:** `departments` (+`is_active`), `users` (+`is_active`), `requests` UPDATE RLS'i veya yeni RPC, YENİ `admin_department_screen.dart`, `request_detail_screen.dart` (yönlendirme aksiyonu), muhtemelen RLS politikalarında `is_active` koşulları.

### Faz 5 — SLA + istatistikler
- Talep çözüm süresi: `requests.resolved_at` (veya onay anı) zaman damgası; talep detayında "X saat/gün içinde çözüldü" gösterimi.
- SLA/eskalasyon: yapılandırılabilir süre (örn. 3 gün) boyunca atanmamış/çözülmemiş talep için müdüre uyarı bildirimi. İki yaklaşım (DB tarafı zamanlanmış kontrol + notifications vs uygulama açılışında kontrol) karşılaştırılıp biri önerilecek.
- Admin istatistik ekranı (toplam talep, çözülen/çözülmeyen, birim bazlı sayılar) + müdür istatistik ekranı (aynısının kendi birimi versiyonu).
- **Etkilenen yerler:** `requests` (+`resolved_at` + durum geçiş trigger'ı), muhtemelen bir SLA ayar tablosu, `notifications`, YENİ istatistik ekranları (`admin_stats_screen.dart`, `manager_stats_screen.dart`), muhtemelen aggregate RPC fonksiyonları (RLS ile uyumlu sayım için).
